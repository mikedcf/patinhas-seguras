    ...
    try {
        ...
        if (dados.logado) {
            ...
            if(dados.usuario.tipo == 'admin'){
                document.getElementById('btn-open-add-modal').style.display = 'block'
            }
            else{
                document.getElementById('btn-open-add-modal').style.display = 'none'
            }
        }
        else {
...
function queroAdotar() {
    ...
    document.getElementById('btn-quero-adotar').textContent = '✔️ Adoção solicitada!';
    document.getElementById('btn-quero-adotar').style.backgroundColor = 'rgb(29, 243, 9)';

    notify('Sucesso!', '✔️ Adoção solicitada com sucesso!', 2000, null, 'success');

    setTimeout(() =>{
        closeDetailsModal()
        document.getElementById('btn-quero-adotar').textContent = 'Quero adotar!';
        document.getElementById('btn-quero-adotar').style.backgroundColor = '#f97316';
    }, 2000)
...
function handleAddAnimalFormSubmit(event) {
...
function initializeAddAnimalModal() {
    const addAnimalModal = document.getElementById('add-animal-modal');
    const btnOpenAddModal = document.getElementById('btn-open-add-modal');
    const addAnimalForm = document.getElementById('add-animal-form');

    if (!addAnimalModal || !btnOpenAddModal || !addAnimalForm) return;
    ...
    addAnimalForm.addEventListener('submit', handleAddAnimalFormSubmit);
}
...
function criarCardAnimal(data) {
    ...
    const carrosselContainer = isCachorro ? 
        document.querySelector('#secao-cachorros .carrossel') : 
        document.querySelector('#secao-gatos .carrossel2');
    ...
    if (carrosselContainer) {
        ...
        newCard.addEventListener('click', () => {
            openDetailsModal(newCard.dataset);
        });
    }
}
